...
class QuestionComponent extends Component {
  ...
  /*Makes validation on limiter inputs*/
  ...
  /*Add answer button is disabled till new answer is successfully inserted. Then it is enabled again*/
  enableAddAnswerBtn() {
  ...
  /*If answers are empty, then question's limiter section must be initialized*/
          ...
          this.state.question.answers.map((answer, index) => {
            return (
              <AnswerComponent
                key={answer.id}
                isLast={
                  index + 1 === this.state.question.answers.length
                    ? true
                    : false
                }
                index={index}
      ...
      <div>
        <fieldset className="question">
          ...
          <section className="question-settings">
              ...
              <Col xs={12}>
                <InputGroup>
                  <FormControl
                    ...
                    value={this.state.question.prompt}
                    onChange={this.updateQuestionPrompt}
                  />
                  <InputGroup.Append className="question-controls">
                    <Button
                      variant="outline-secondary"
                      onClick={() => this.moveQuestionUpwards(this.props.index)}
                    ...
                    <Button
                      variant="outline-secondary"
                      onClick={() =>
                        this.moveQuestionDownwards(this.props.index)
                      }
            ...
            <Row
              className={
                'question-settings-limit ' +
                (!this.state.question.answers.filter(
                  answer => answer.isInserted
                ).length > 0
                  ? 'hidden'
                  : '')
              }
            >
              ...
              <Col
                ...
                className={
                  'p-0 ' +
                  (!this.state.question.limitAnswers.enabled ? 'hidden' : '')
                }
              >
                ...
                <div
                  className={
                    'error-msg limit-answer-error ' +
                    (this.errors.exist ? '' : 'hidden')
                  }
                >
                  <span
                    className={
                      this.errors.limitAtLeast.rangeOverflow ? '' : 'hidden'
                    }
                  >
                    <i className="fas fa-exclamation-circle"></i> 'At Least'
                    value can't be greater than 'No more than' value
                  </span>
                  <span
                    className={
                      this.errors.limitAtLeast.rangeUnderflow ? '' : 'hidden'
                    }
                  >
                    <i className="fas fa-exclamation-circle"></i> 'At Least'
                  ...
                  <span
                    className={
                      this.errors.limitAtLeast.badInput ? '' : 'hidden'
                    }
                  >
                    <i className="fas fa-exclamation-circle"></i> Invalid 'At
                    Least' value
                  </span>
                  <span
                    className={
                      this.errors.limitNoMoreThan.rangeUnderflow ? '' : 'hidden'
                    }
                  >
                    <i className="fas fa-exclamation-circle"></i> 'No more than'
                    value can't be lesser than 'At Least' value
                  </span>
                  <span
                    className={
                      this.errors.limitNoMoreThan.badInput ? '' : 'hidden'
                    }
                  >
                    <i className="fas fa-exclamation-circle"></i> Invalid 'No
                    more than' value
                  </span>
                </div>
              </Col>
          ...
          <section className="question-answers">
            <Row>
              <Col sm={12}>
                <h6
                  className={
                    this.state.question.answers.length === 0 ? 'hidden' : ''
                  }
                >
                  Answers
                </h6>
                {this.state.question.answers.length > 0 ? (
                  this.state.question.answers.map((answer, index) => {
                    let visibleLength = this.state.question.answers.filter(
                      ans => ans.visible
                    ).length;
                    if (answer.visible) {
                      return (
                  ...
                  <p className="no-answers-display-msg">
                    Create a new answer by clicking the 'Add a potential answer'
                    button!
                  </p>
                )}
              </Col>
            ...
            <Row>
              <Col>
                <Button
                  ...
                  disabled={this.state.disableAddAnswerBtn}
                  onClick={this.addAnswer}
                >
                  <i className="fas fa-plus"></i> Add a potential answer
                </Button>
              </Col>
            </Row>
          </section>
